...
from plotting_utils_ntex import load_glmhmm_data, load_animal_list, load_cv_arr, \
...
if __name__ == '__main__':
    ...
    input_ver = 'random'
    data_dir = '/home/craig/Neurodynamic-control-toolbox/NT-GLM-HMM/data/lapse/'+input_ver+'/'
    # overall_dir = '../../results/ibl_individual_fit/'
    overall_dir = '/home/craig/Neurodynamic-control-toolbox/NT-GLM-HMM/data/lapse/'+input_ver+'/'
    # animal_list = load_animal_list(data_dir + 'animal_list.npz')
    sigma_val = 2
    alpha_val = 2
    npr.seed(67)
    animal_list = [240,241,242,243]
    # input_ver = 'last_trial_behavior_no_bias'
    
    for animal in animal_list:
        results_dir = overall_dir
        ...
        for num_lapse_params in [2]:
            if num_lapse_params == 1:
                lapse_file = results_dir + \
                                '/Lapse_Model/' + \
                                    str(animal) + '/fold_0/lapse_model_params_one_param.npz'
            elif num_lapse_params == 2:
                lapse_file = results_dir + \
                                '/Lapse_Model/' + \
                                    str(animal) + '/fold_0/lapse_model_params_two_param.npz'
        ...
        print(animal)
        print(np.mean(predictive_acc_lapse))
