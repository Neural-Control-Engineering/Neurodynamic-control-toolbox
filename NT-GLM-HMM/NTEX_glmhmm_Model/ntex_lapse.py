...
if __name__ == '__main__':
    ...
    input_ver = 'spontaneous_mpfc_stim'
    # input_ver = 'random'
    results_dir = '/home/craig/Neurodynamic-control-toolbox/NT-GLM-HMM/data/lapse/v3/' + input_ver + '/'
    ...
    for animal in animals:
        ...
        loaded = loadmat('/home/craig/Neurodynamic-control-toolbox/NT-GLM-HMM/data/lapse/v3/'+input_ver+'/'+str(animal)+'_' + input_ver + '.mat')
        inpt = loaded["preprocessed_input"]
        y = loaded["preprocessed_label"] 
        y = y.astype('int')     
        
        ## replacement for session_fold_lookup_table
        inds = np.random.permutation(y.shape[0])
        fold_lookup = np.empty(inds.shape)
        start = 0
        plus = fin = int(np.ceil(inds.shape[0] / num_folds))
        for fold in range(num_folds):
            fold_lookup[inds[start:fin]] = fold
            start = fin 
            fin = fin + plus
        ...
        for fold in range(num_folds):
            ...
            labels_for_plot = ['stim_strength', 'prev_response', 'reward_state', 'bias']
            # y = y.astype('int')
            ...
            figure_directory = results_dir + "Lapse_Model/" + str(animal) + "/fold_" + str(fold) + '/'
