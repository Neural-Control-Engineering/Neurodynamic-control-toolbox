function fig4c(data, tbounds, alignTo, ver)
    ...
    for s = 1:length(stim_strengths)
        ...
        [~, s1, t] = avg_photo_traces(stmp, [tbounds(1), tbounds(2)], alignTo, ver);
        s1 = s1(:,2:end-1);
        t = t(2:end-1);
        b =  nanmean(s1(:,(t > -0.5 & t < 0)),2);
        % e = nanmean(s1(:,(t > 0 & t < 2)),2);
        e = max(s1(:,(t > 0 & t < 2)),[],2);
        d = e - b;
    ...
    % xlim([-2.5,5.1])
    saveas(fig, 'Figures/fig4c.fig')
    saveas(fig, 'Figures/fig4c.svg')
