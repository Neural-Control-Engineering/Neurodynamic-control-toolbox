function plotPhases(data, sortBy, t0, t1, outdir)
    ...
    for out_i = 1:length(outcome_types)
        ...
        if ~isempty(obj)
            ...
            for i = 1:size(obj,1)
                ...
                analyticSignal1 = hilbert(ch1);
                analyticSignal2 = hilbert(ch2);
                phases(1,i) = angle(analyticSignal1(end)); % Using end for the phase at t1
                phases(2,i) = angle(analyticSignal2(end));

            end
            if strcmp(sortBy, 'outcome')
                subplot(2,1,1); hold on;
                scatter(ones(1,size(obj,1))+out_i, phases(1,:), 15, colors(out_i), "filled", 'jitter', 'on', 'DisplayName', outcome)
                subplot(2,1,2); hold on;
                scatter(ones(1,size(obj,1))+out_i, phases(2,:), 15, colors(out_i), "filled", 'jitter', 'on', 'DisplayName', outcome)
            elseif strcmp(sortBy, 'response')
                subplot(2,1,1); hold on;
                scatter(ones(1,size(obj,1))+out_i, phases(1,:), 15, colors(out_i), "filled", 'jitter', 'on', 'DisplayName', responses{out_i})
                subplot(2,1,2); hold on;
                scatter(ones(1,size(obj,1))+out_i, phases(2,:), 15, colors(out_i), "filled", 'jitter', 'on', 'DisplayName', responses{out_i})
            end
        end
    end
