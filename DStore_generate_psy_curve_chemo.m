function [figure_out] = DStore_generate_psy_curve_chemo(DStore_path,Extra_filter, plot_title)
...
all_chemo = gather(Datastore(all_nonempty_index,26));
lg_CNO = contains(all_chemo, 'CNO');
lg_Saline = contains(all_chemo, 'Saline');
lg_psy_curve_CNO = lg_psy_curve & lg_CNO;
lg_psy_curve_Saline = lg_psy_curve & lg_Saline;

target_psy_curve_CNO = all_psy_curve(lg_psy_curve_CNO, 1);
target_psy_curve_Saline = all_psy_curve(lg_psy_curve_Saline, 1);
%target_photometry = all_psy_curve(lg_photometry,1);
%%
figure_out = figure();
hold on
sum_curve = zeros(numel(target_psy_curve_CNO),numel(target_psy_curve_CNO{1}(2,:)));
for i = 1:numel(target_psy_curve_CNO)
    p1 = plot(target_psy_curve_CNO{i}(1,:), ...
        target_psy_curve_CNO{i}(2,:),'b','LineWidth',1.2);
    p1.Color(4) = 0.15;
    sum_curve(i,:) = target_psy_curve_CNO{i}(2,:);
end
mean_curve = mean(sum_curve);
SEM_curve = std(sum_curve)/sqrt(height(sum_curve));
p2 = errorbar(target_psy_curve_CNO{1}(1,:),mean_curve,SEM_curve, ...
    'b','LineWidth',1.7);

sum_curve = zeros(numel(target_psy_curve_Saline),numel(target_psy_curve_Saline{1}(2,:)));
for i = 1:numel(target_psy_curve_Saline)
    p3 = plot(target_psy_curve_Saline{i}(1,:), ...
        target_psy_curve_Saline{i}(2,:),'r','LineWidth',1.2);
    p3.Color(4) = 0.15;
    sum_curve(i,:) = target_psy_curve_Saline{i}(2,:);
end
plot_title2 = strcat(plot_title,' (CNO, ',num2str(numel(target_psy_curve_CNO)), ...
    ' Sessions, Saline, ',num2str(numel(target_psy_curve_Saline)),' Sessions)');
mean_curve = mean(sum_curve);
SEM_curve = std(sum_curve)/sqrt(height(sum_curve));
p4 = errorbar(target_psy_curve_Saline{1}(1,:),mean_curve,SEM_curve, ...
    'r','LineWidth',1.7);
legend([p2, p4], "CNO Sessions","Saline sessions",'Location','Southeast')
title(plot_title2)
