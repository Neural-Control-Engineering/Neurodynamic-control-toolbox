...
%for shaded error bars
addpath('C:\Users\Gabog\Downloads\shadedErrorBar') %should just add to repo

% Set variables here
opt_windows.response_window = 1; %seconds
opt_windows.before_event = 2; %seconds
opt_windows.after_event = 5; %seconds
...
% CHECK PERFORMANCE
session_list = fetchSessions(paths,animals_to_load);
% pupilData = checkPupilDataNoBehav(paths,session_list,analysis_params);
[NTData, time] = checkPhotometryData(paths, session_list, analysis_params);
