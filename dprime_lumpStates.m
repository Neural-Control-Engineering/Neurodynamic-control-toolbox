        ...
        for as = 1:size(ordered_states,1)
            ...
            if ~isempty(statetmp)
                if strcmp(psychver, 'byanimal')
                    ...
                    sstmp = filterTrials(statetmp, 'stim_strength', 0);
                    fatmp = filterTrials(sstmp, 'categorical_outcome', 'FA');
                    far = (size(fatmp,1)+0.5) / (size(sstmp,1)+1);
                    for ss = 2:length(tmp_strengths)
                        ...
                        ind = find(stim_strengths(2:end) == tmp_strengths(ss));
                        hr = (size(rtmp,1)+0.5) / (size(sstmp,1)+1.0);
                        rp(ind) = norminv(hr)-norminv(far);
                    end
                end
                state_rp = [state_rp; rp];
            end
        end
        semshade(state_rp, 0.3, cols(s,:), cols(s,:), stim_strengths(2:end) .* 10);
        hold on
    end
