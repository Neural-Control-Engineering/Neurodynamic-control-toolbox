...
function all_mat = lumpByResponseProb(data_ver, ssd_version, psychver, animals, data, k, folds)
    ...
    figure();
    tl = tiledlayout(1,size(all_mat,3));
    axs = zeros(1,size(all_mat,3));
    for i = 1:size(all_mat,3)
        axs(i) = nexttile;
        imagesc(0:k-1, 0:k-1, all_mat(:,:,i))
        xticks([0:4])
        yticks([0:4])
        title(sprintf('Animal %i', i))
    end
    cbar = colorbar();
    ylabel(cbar, 'Transition Probability', 'FontSize', 16)
    ylabel(tl, 'Current State', 'FontSize', 16)
    xlabel(tl, 'Next State', 'FontSize', 16)
end
