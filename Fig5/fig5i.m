function fig5i(data, k, data_ver, ssd_version, psychver, animals)
...
function lumpByResponseProb(data_ver, ssd_version, psychver, animals, data, k, folds)
    ...
    weights_animal = zeros(k,2,length(animals));
    for s = 1:k
        for as = 1:size(ordered_states,1)
            ...
            for ii = 1:5
                w = reshape(results.hmm_params{ii,3},[k, n_features]);
                for ik = 1:k
                    weights(ik, :, ii) = fliplr(w(ik,:));
                end
            end
            % if any(abs(weights(:)) > 45)
            %     keyboard 
            % end
            weights_animal(s,:,as) = mean(weights(s,:,:),3);
        end
    end
    ...
    stim_weights = [];
    pupil_weights = [];
    for s = 1:size(weights_animal,1)
        wa = weights_animal(s,1,:);
        x = zeros(1,size(weights_animal,3))+s-0.1+(rand(1,size(weights_animal,3))-0.5)*0.1;
        if s == 1
            plot(x, wa(:), 'o', 'MarkerFaceColor', [0.3,0.3,0.3], 'MarkerEdgeColor', [1,1,1], 'MarkerSize', 10, 'DisplayName', 'Pupil Area')
        else
            plot(x, wa(:), 'o', 'MarkerFaceColor', [0.3,0.3,0.3], 'MarkerEdgeColor', [1,1,1], 'MarkerSize', 10, 'HandleVisibility', 'off')
        end
        errorbar(s-0.1, mean(wa(:)), ste(wa(:)), 'k.', 'CapSize', 15, 'LineWidth', 2, 'HandleVisibility', 'off')
        pupil_weights = [pupil_weights, wa(:)];
        wa = weights_animal(s,2,:);
        x = zeros(1,size(weights_animal,3))+s+0.1+(rand(1,size(weights_animal,3))-0.5)*0.1;
        if s == 1
            plot(x, wa(:), 'o', 'MarkerFaceColor', [0.6,0.6,0.6], 'MarkerEdgeColor', [1,1,1], 'MarkerSize', 10, 'DisplayName', 'Stimulus Intensity')
        else
            plot(x, wa(:), 'o', 'MarkerFaceColor', [0.6,0.6,0.6], 'MarkerEdgeColor', [1,1,1], 'MarkerSize', 10, 'HandleVisibility', 'off')
        end
        errorbar(s+0.1, mean(wa(:)), ste(wa(:)), 'k.', 'CapSize', 15, 'LineWidth', 2, 'HandleVisibility', 'off')
        stim_weights = [stim_weights, wa(:)];
    end
    tcks = [];
    tls = {};
    count = 1;
    for s = 1:size(weights_animal,1)
        tcks = [tcks, s-0.1, s+0.1];
        tls{count} = 'Pupil Area';
        count = count + 1;
        tls{count} = 'Stim. Intensity';
        count = count + 1;
    end
    xticks(1:4)
    xlabel('State')
    ...
    % xticklabels(tls)
    % xtickangle(45)
    fprintf('Stimulus Weights:\n')
    [p,tbl,stats] = anova1(stim_weights)
    fprintf('Pupil area weights:\n')
    [p,tbl,stats] = anova1(pupil_weights)
    saveas(fig, 'Figures/fig5i.fig')
    saveas(fig, 'Figures/fig5i.svg')
end
