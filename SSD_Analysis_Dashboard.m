...
for animal_number = 1:length(animals_to_load)
    tic
    % Update wait bar
    waitbar(completed_animals/total_animals,wbar,[num2str(completed_animals) '/' num2str(total_animals) ' animals completed']);

    % Find how many unique sessions a single animal has
    [session_names] = retrieve_session_list(animals_to_load{animal_number},raw_behavior_data_path);
    temp_phase = "";
    ...
    for session = 1:height(session_names)
        ...
        cd(raw_behavior_data_path)
        session_behavior_data = load(session_names{session,1});

         % Fetch behavior related data for the session
        [training_phase, behavior_data] = fetch_behavior(session_behavior_data,start_buffer,end_buffer);
        % Calculate frame count and start/end frame values for the session
        [video_details] = setSessionParameters(session_behavior_data,start_buffer,end_buffer,pupil_fs,whisker_fs);

        if ~strcmp(temp_phase,training_phase)
            session_count = 1;
            temp_phase = training_phase;
        else
        ...
        % ################ PUPIL #####################
        % Load pupil videos
        cd(analyzed_pupil_data_path);
        cd(session_names{session,3});
        pupil_to_load = dir(strcat('*',session_names{session,5},session_names{session,6},session_names{session,4},'*'));
        if ~isempty(pupil_to_load)
            pupil_data = readtable(pupil_to_load.name);
            % Trim pupil data according to start/end times
            pupil_data = pupil_data(pupil_data.scorer >= video_details.pupil(1,1) & pupil_data.scorer <= video_details.pupil(1,2),:);
        else
            pupil_data = [];
        end

        % ################ PHOTOMETRY #####################
        % Find photometry data for a given session
        cd(raw_photometry_data_path);
        photometry_to_load = dir(strcat('*', session_names{session,2},'_',session_names{session,4},'_',session_names{session,5},'_',session_names{session,6},'*'));
        if ~isempty(photometry_to_load)
            photometry_data = readtable(photometry_to_load.name);
        else
            photometry_data = [];
        end

        % ################ ANIMAL SUMMARY ##################
        % For a given animal, create a cell array with relevant info for
        % each session


        [animal_summary_of_sessions] = returnTrialsFromSession(response_window,baseline,time_before_stim,time_after_stim,puff_values,num_of_properties,start_buffer,end_buffer,pupil_fs,photometry_fs,whisker_fs,video_details,training_phase,session_count,session_names(session,:),behavior_data,pupil_data,photometry_data);
    ...
    combined_summary_of_sessions = vertcat(combined_summary_of_sessions,animal_summary_of_sessions);

    end

    

    % Update count for wait bar
    completed_animals = completed_animals + 1;
    toc
end
...
combined_summary_of_sessions = combined_summary_of_sessions(find(~cellfun(@isempty,combined_summary_of_sessions(:,1))),:);
%% Load data
