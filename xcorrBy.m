function xcorrBy(data, filterBy, filterValue, sortBy, alignTo, tbounds, outdir)
...
% on properties like the animal, session_id, stimulus strength, etc. 
% (see filterTrials.m). alignTo specifies wheter to align trials to the 
% stimulus_time or response_time 
    ...
    if strcmp(sortBy, 'outcome')
        ...
        for out_i = 1:length(outcome_types)
            ...
            if ~isempty(data_outcome)
                ...
                cs = zeros(size(data_outcome,1), round(Fs*2*diff(tbounds)-1));
                lags = zeros(size(data_outcome,1), round(Fs*2*diff(tbounds)-1));
