function [behavior] = fetch_phase_III_behavior(Data,start_buffer,end_buffer)
...
behavior.trial_onset_tone(:,1) = trial_start_time( ...
    trial_start_time(:,vars)>=start_buffer & ...
    trial_start_time(:,vars)<=trial_start_time(end,vars)-end_buffer,vars)-start_buffer;

if ~isempty(punishment_time)
    behavior.punishment_times(:,1) = punishment_time( ...
        punishment_time(:,vars)>=behavior.trial_onset_tone(1)+start_buffer & ...
        punishment_time(:,vars)<=punishment_time(end,vars)-end_buffer,vars)-start_buffer;
else
    behavior.punishment_times = [];
end

if ~isempty(reward_time) 
    behavior.reward_times(:,1) = reward_time( ...
        reward_time(:,vars)>=behavior.trial_onset_tone(1)+start_buffer & ...
        reward_time(:,vars)<=reward_time(end,vars)-end_buffer,vars)-start_buffer;
else
    behavior.reward_times = [];
end

behavior.stim_strength(:,1) = stim_time( ...
    stim_time(:,vars)>=behavior.trial_onset_tone(1)+start_buffer & ...
    stim_time(:,vars)<=stim_time(end,vars)-end_buffer,4);
behavior.stim_times(:,1) = stim_time( ...
    stim_time(:,vars)>=behavior.trial_onset_tone(1)+start_buffer & ...
    stim_time(:,vars)<=stim_time(end,vars)-end_buffer,vars)-start_buffer;
if ~isempty(lick_time)
    behavior.lick_times(:,1) = lick_time( ...
        lick_time(:,vars)>=behavior.trial_onset_tone(1)+start_buffer & ...
        lick_time(:,vars)<=lick_time(end,vars)-end_buffer,vars)-start_buffer;
else
    behavior.lick_times = [];
end
behavior.distractor_times(:,1) = distractor_time( ...
    distractor_time(:,vars)>=behavior.trial_onset_tone(1)+start_buffer & ...
    distractor_time(:,vars)<=distractor_time(end,vars)-end_buffer,vars)-start_buffer;
