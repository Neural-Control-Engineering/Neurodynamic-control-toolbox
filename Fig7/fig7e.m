function fig7e(data, k, data_ver, ssd_version, psychver, animals)
...
function [sesh_ps, all_sesh] = lumpByResponseProb(data_ver, ssd_version, psychver, animals, data, k)
        ...
        for as = 1:size(ordered_states,1)
            ...
            if ~isempty(statetmp)
                ...
                for o = 1:length(outcomes)
                    ...
                    if ~isempty(otmp)
                        [ch1, ~, t] = avg_photo_traces(otmp, [-0.5,0], 'stimulus','filtered');
                        ne_ch1{o} = [ne_ch1{o}; nanmean(ch1,2)];
    ...
    ylabel(tl, 'Baseline NE_{mPFC} (z-score)', 'FontSize', 16)
    saveas(fig, 'Figures/fig7e.fig')
    saveas(fig, 'Figures/fig7e.svg')
end
