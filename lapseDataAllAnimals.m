...
for i = 1:length(data_versions)
    ...
    outdir = sprintf('NT-GLM-HMM/data/lapse/v3/%s/', data_version);
    if ~exist(outdir, 'dir')
        mkdir(outdir)
    end
    ...
    for a = 1:length(animals)
        ...
        genLapseData(tmp, sprintf('%s%s_%s.mat', outdir, animal, data_version), data_version, false)
    end
end

function genLapseData(data, outfile, version, shuffle, seed)
...
% Craig Kelley, NEC Lab, 8/21/23
    ...
    elseif strcmp(version, 'spontaneous_s1_stim')
        ...
        preprocessed_label = num2cell(data.go_nogo);
        preprocessed_trial_number = data.sequential_trial_number;
    elseif strcmp(version, 'random')
        metrics = getSpontaneousMetrics(data, true);
        stim_strengths = data.stimulus_strength ./ max(data.stimulus_strength);
        metrics = metrics(:,5);
        preprocessed_input = [rand(size(data,1),1), stim_strengths];
        preprocessed_session = sessions;
        preprocessed_label = num2cell(data.go_nogo);
        preprocessed_trial_number = data.sequential_trial_number;
    ...
    elseif strcmp(version, 'last_trial_behavior_no_bias')
        ...
        reward_states = reward_states(1:end-1);
        strength = stim_strengths(2:end);
        preprocessed_input = [strength, responses, reward_states];
        preprocessed_session = sessions;
    ...
    elseif strcmp(version, 'dynamic_state')
        for i = 1:length(sessions)
            ...
            if shuffle
                ...
                preprocessed_input{i,1} = [metrics, strength];
            end
            preprocessed_session{i,1} = sessions{i};
            preprocessed_label{i,1} = num2cell(tmp.go_nogo);
            preprocessed_trial_number{i,1} = tmp.sequential_trial_number; 
